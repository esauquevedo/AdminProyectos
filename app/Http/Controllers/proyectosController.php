...
class proyectosController extends Controller
{
    ...
    public function pdf(){
     ...
     public function guardarAsignacion($id, Request $datos){
      ...
      public function asignarRecursoPDF($id) {
     $asignarRecurso=DB::table ('proyectos_recursos')
    ->join('recursos','proyectos_recursos.recurso_id','=','recursos.id')
    ->join('proyectos','proyectos_recursos.proyecto_id','=','proyectos.id')
    ->where('proyectos.id','=',$id)
    ->select('proyectos.descripcion','recursos.nombre')
    ->get();
        $proyectos=Proyecto::all();
        $vista=view('asignarRecursoPDF', compact('asignarRecurso'));
        $pdf=\App::make('dompdf.wrapper');
        $pdf->loadHTML($vista);
        $pdf->setPaper('letter');
        flash('!Usted cerro Archivo PDF!')->success();
